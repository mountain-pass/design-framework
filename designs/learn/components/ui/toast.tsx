...
function Toast({ className, variant = "default", ...props }: React.ComponentProps<typeof ToastPrimitive.Root> & { variant?: "default" | "destructive" }) {
  return (
    <ToastPrimitive.Root
      data-slot="toast"
      className={cn(variant === "destructive" ? "flex items-start gap-3 rounded-xl border border-destructive/30 bg-popover p-4 text-popover-foreground shadow-lg" : "flex items-start gap-3 rounded-xl border-2 border-border bg-popover p-4 text-popover-foreground shadow-lg", className)}
      {...props}
    />
  )
}
...
function ToastClose({ className, ...props }: React.ComponentProps<typeof ToastPrimitive.Close>) {
  return <ToastPrimitive.Close data-slot="toast-close" className={cn("-mr-1 -mt-1 inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", className)} {...props} />
}

function ToastAction({ className, ...props }: React.ComponentProps<typeof ToastPrimitive.Action>) {
  return <ToastPrimitive.Action data-slot="toast-action" className={cn("inline-flex h-7 shrink-0 items-center rounded-lg px-2.5 text-xs font-medium text-primary transition-colors hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", className)} {...props} />
}
