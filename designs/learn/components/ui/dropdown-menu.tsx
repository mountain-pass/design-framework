  ...
  return (
    <DropdownMenuPrimitive.Portal>
      <DropdownMenuPrimitive.Content
        ...
        className={cn("rounded-xl border-2 border-border bg-popover p-1 text-popover-foreground shadow-md z-50 min-w-[8rem] overflow-hidden", className)}
        {...props}
      />
    </DropdownMenuPrimitive.Portal>
  )
...
function DropdownMenuItem({ className, ...props }: React.ComponentProps<typeof DropdownMenuPrimitive.Item>) {
  return (
    <DropdownMenuPrimitive.Item
      ...
      className={cn("flex h-9 w-full items-center gap-2.5 rounded-sm px-2 text-sm transition-colors hover:bg-accent hover:text-accent-foreground", "cursor-default outline-none focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50", className)}
      {...props}
    />
  )
}
