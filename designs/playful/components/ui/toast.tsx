...
function Toast({ className, variant = "default", ...props }: React.ComponentProps<typeof ToastPrimitive.Root> & { variant?: "default" | "destructive" }) {
  return (
    <ToastPrimitive.Root
      data-slot="toast"
      className={cn(variant === "destructive" ? "flex items-start gap-3 rounded-xl border border-destructive/30 bg-popover p-4 text-popover-foreground shadow-lg" : "flex items-start gap-3 rounded-xl border border-border bg-popover p-4 text-popover-foreground shadow-lg", className)}
      {...props}
    />
  )
}
